...
class TestGit(unittest.TestCase):
    ...
    def test_init_credentials_private(self):
        ...
        self.repo = git.Git(self.repo_url, self.ref, self.folder, {'public': 'mypublicpath', 'private': 'mykeypath', 'passphrase': 'mypassphrase'})
        self.assertEqual(vars(self.repo.credentials),vars(credentials))


    @patch('git.os.path.isdir', return_value=False)
    @patch('git.pygit2.Repository')
    @patch('git.pygit2.clone_repository')
    def test_clone(self, mock_clone_repository, mock_pygit2, mock_isdir):
        self.repo = git.Git(self.repo_url, self.ref, self.folder, {'user': self.username, 'password': self.password}, self.auto)
    ...
    def test_clone_ref_is_tag(self, mock_repository, mock_clone_repository):
        ...
        self.repo = git.Git(self.repo_url, 'mytag', self.folder, {'user': self.username, 'password': self.password}, self.auto)
        self.assertTrue(self.repo.ref_is_tag())
        self.repo = git.Git(self.repo_url, 'mybranch', self.folder, {'user': self.username, 'password': self.password}, self.auto)
        self.assertFalse(self.repo.ref_is_tag(), True)


    @patch('git.pygit2.Repository')
    @patch('git.pygit2.Remote')
    @patch('git.Git.checkout')
    def test_is_remote(self, mock_checkout, mock_remote, mock_repository):
        ...
        self.repo = git.Git('https://gitlab.suse.de/juliogonzalezgil/terracumber2.git', 'mytag', self.folder, {'user': self.username, 'password': self.password}, self.auto)
    ...
    @patch('git.os.path.isdir', return_value = True)
    @patch('git.pygit2.Repository')
    @patch('git.Git.refresh_local_repo', return_value=('https---gitlab-suse-de-juliogonzalezgil-terracumber-git', 'https://gitlab.suse.de/juliogonzalezgil/terracumber.git'))
    def test_refresh_local_repo(self, mock_refres_local_repo, mock_repository, mock_is_dir):
        ...
        with self.assertRaises(Exception) as e:
            self.repo = git.Git('https://gitlab.suse.de/juliogonzalezgil/terracumber.git', 'mytag', self.folder, {'user': self.username, 'password': self.password}, self.auto)
        self.assertEqual(str(e.exception), "Could not find reference refs/tags/mytag (remote URL https://gitlab.suse.de/juliogonzalezgil/terracumber.git)")
        # Branch exists locally but not on the remote
        with self.assertRaises(Exception) as e:
            self.repo = git.Git('https://gitlab.suse.de/juliogonzalezgil/terracumber.git', 'master', self.folder, {'user': self.username, 'password': self.password}, self.auto)
        self.assertEqual(str(e.exception), "Could not find reference refs/remotes/https---gitlab-suse-de-juliogonzalezgil-terracumber-git/master (remote URL https://gitlab.suse.de/juliogonzalezgil/terracumber.git)")
        ...
        self.repo = git.Git('https://gitlab.suse.de/juliogonzalezgil/terracumber.git', 'mybranch', self.folder, {'user': self.username, 'password': self.password}, self.auto)
