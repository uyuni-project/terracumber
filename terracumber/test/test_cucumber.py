...
class TestGit(unittest.TestCase):
    ...
    @patch('cucumber.paramiko.SSHClient')
    @patch('builtins.open')
    def test_run_command(self, mock_open, mock_sshclient):
        ...
        mock_sshclient.return_value.get_transport.return_value.open_session.return_value.makefile.return_value = ['VAR=555']
    ...
    @patch('cucumber.paramiko.SSHClient')
    @patch('cucumber.paramiko.sftp_attr')
    @patch('cucumber.os.utime')
    ...
    @patch('cucumber.paramiko.SSHClient')
    @patch('cucumber.Cucumber.copy_atime_mtime')
    def test_get(self, mock_copy_atime_mtime, mock_sshclient):
        ...
        self.assertEqual(self.cucumber.get('/remote_path/.?file.*','/local_path/'), ['/remote_path/file', '/remote_path/afile', '/remote_path/file2'])
        # Get a whole directory
        self.assertEqual(self.cucumber.get('/remote_path/.*','/local_path/'), ['/remote_path/file', '/remote_path/afile', '/remote_path/aafile', '/remote_path/file2'])


    @patch('cucumber.paramiko.SSHClient')
    @patch('cucumber.Cucumber.copy_atime_mtime')
    @patch('cucumber.os.mkdir')
    def test_get_recursive(self, mock_mkdir, mock_copy_atime_mtime, mock_sshclient):
        mock_sshclient.return_value.open_sftp.return_value.listdir.return_value = ['file','file2','folder/file3', 'folder/subfolder/file4']
        self.cucumber = cucumber.Cucumber(self.conn_data)
        ...
        mock_sshclient.return_value.open_sftp.return_value.get.called_with('/remote_path/folder/file3','/local_path/folder/file3')
        mock_mkdir.called_with('/local_path/folder/subfolder')
        mock_sshclient.return_value.open_sftp.return_value.get.called_with('/remote_path/folder/subfolder/file4','/local_path/folder/subfolder/file4')
